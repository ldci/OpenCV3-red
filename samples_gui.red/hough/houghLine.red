...
[
	...
	;CV_HOUGH_STANDARD :lines/elem_size = 8 
	
	either method = 0 [
		lines: cvHoughLines2 dst &storage method distance 
		angle threshold 0.0 0.0 minTheta maxTheta
	] [
		lines: cvHoughLines2 dst &storage method distance 
...
showHough: does [
	if isFile [
		img2: computeHough flagValue distance angle thresh param1 param2 minAngle maxAngle
...
view win: layout [
	...
	flag: drop-down 200x24 
		data ["CV_HOUGH_STANDARD" "CV_HOUGH_MULTI_SCALE"] 
	    select 1 
	    on-change [
	    	flagValue: flag/selected - 1
	    	if flagValue = 1 [flagValue: 2]
	    	showHough
	    ]   
		 
	text "Threshold" 70 
	sl1: slider 250 [thresh: 1 + to-integer round face/data * (min wsz hsz) 
							fThresh/text: form thresh
							showHough
						]
	fThresh: field 40 "50"
